...
class MediaLibraryService extends FletService {
  ...
  Future<dynamic> _onInvokeMethod(String methodName, dynamic args) async {
    final arguments =
        (args != null && args is Map)
            ? Map<String, dynamic>.from(args)
            : <String, dynamic>{};
    return switch (methodName) {
      // permissions
      "check_permissions" => _guard(() => _adapter.checkPermissions(
          _mediaTypes(arguments),
        )),
      "request_permissions" => _guard(() => _adapter.requestPermissions(
          _mediaTypes(arguments),
        )),
      ...
      "get_albums" => _guard(() async {
          ...
          return {"albums": [for (final a in albums) a.toMap()]};
        }),
      "get_assets" => _guard(() => _adapter.getAssets(
      ...
      "get_asset" => _guard(() async {
          final asset =
              await _adapter.getAsset(arguments["asset_id"] as String? ?? "");
          return asset.toMap();
        }),
      ...
      "delete_assets" => _guard(() async {
          final ids = (arguments["asset_ids"] as List?)
                  ?.map((e) => e.toString())
                  .toList() ??
              const [];
          if (ids.isEmpty) {
